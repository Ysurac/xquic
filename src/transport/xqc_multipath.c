...
xqc_int_t
xqc_conn_set_path_dscp_mask(xqc_engine_t *engine, const xqc_cid_t *cid,
    uint64_t path_id, uint64_t dscp_mask)
{
    ...
    path->path_dscp_mask = dscp_mask;
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|set_path_dscp_mask|path_id:%ui|dscp_mask:%ui|", path_id, dscp_mask);
    return XQC_OK;
}
