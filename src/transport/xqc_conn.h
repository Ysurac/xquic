...
/* xqc_conn_flag_t holds bit values up to bit 42 (RETRY_SENT). MSVC's C
 * compiler always uses int as the underlying type for plain enum, silently
 * truncating any value >= bit 31 — so bits 32..42 (CONN_CLOSING_NOTIFIED,
 * DGRAM_WAIT_FOR_1RTT, LOCAL_TP_UPDATED, PMTUD_PROBING, NO_DGRAM_NOTIFIED,
 * DGRAM_MSS_NOTIFY, MP_WAIT_MP_READY, MP_READY_NOTIFY, HANDSHAKE_DONE_SENT,
 * SERVER_ACCEPT, RETRY_SENT) silently fail to set on Windows builds, which
 * notably breaks multipath's ready_to_create_path callback chain (and PMTUD,
 * HANDSHAKE_DONE_SENT, etc.). Use a uint64_t typedef + #define so the
...
_Static_assert(sizeof(xqc_conn_flag_t) == 8,
               "xqc_conn_flag_t must be 64-bit (MSVC truncates plain enum to int)");
_Static_assert(XQC_CONN_FLAG_MP_WAIT_MP_READY == ((xqc_conn_flag_t)1ULL << 38),
...
typedef struct {
    ...
    xqc_packet_number_t
        first_sent_pktno; /* lowest packet number sent with each key phase */
    xqc_packet_number_t
...
struct xqc_connection_s {
    ...
    xqc_list_head_t conn_write_streams, conn_read_streams, /* xqc_stream_t */
        conn_closing_streams, conn_all_streams;
    ...
    xqc_list_head_t
        undecrypt_packet_in[XQC_ENC_LEV_MAX]; /* buffer for reordered packets */
    ...
    xqc_usec_t first_data_send_time;    /* record the time when the bidirectional stream
                                           first sent data */
    ...
    const xqc_reinj_ctl_callback_t *reinj_callback;
    void *reinj_ctl;
...
xqc_int_t xqc_conn_client_on_alpn(xqc_connection_t *conn, const unsigned char *alpn,
                                  size_t alpn_len);
xqc_int_t xqc_conn_server_on_alpn(xqc_connection_t *conn, const unsigned char *alpn,
                                  size_t alpn_len);

ssize_t xqc_path_send_one_packet(xqc_connection_t *conn, xqc_path_ctx_t *path,
...
xqc_int_t xqc_conn_enc_packet(xqc_connection_t *conn, xqc_path_ctx_t *path,
                              xqc_packet_out_t *packet_out, char *enc_pkt,
                              size_t enc_pkt_cap, size_t *enc_pkt_len,
...
xqc_int_t xqc_conn_send_retry(xqc_connection_t *conn, unsigned char *token,
                              unsigned token_len);
...
xqc_int_t xqc_conn_check_token(xqc_connection_t *conn, const unsigned char *token,
                               unsigned token_len);
xqc_int_t xqc_conn_gen_token(xqc_connection_t *conn, unsigned char *token,
                             unsigned *token_len);
...
xqc_int_t xqc_conn_buff_undecrypt_packet_in(xqc_packet_in_t *packet_in,
                                            xqc_connection_t *conn,
                                            xqc_encrypt_level_t encrypt_level);
xqc_int_t xqc_conn_process_undecrypt_packet_in(xqc_connection_t *conn,
                                               xqc_encrypt_level_t encrypt_level);
...
char *xqc_local_addr_str(xqc_engine_t *engine, const struct sockaddr *local_addr,
                         socklen_t local_addrlen);
char *xqc_peer_addr_str(xqc_engine_t *engine, const struct sockaddr *peer_addr,
                        socklen_t peer_addrlen);
...
/* from send_ctl */
void xqc_conn_decrease_unacked_stream_ref(xqc_connection_t *conn,
                                          xqc_packet_out_t *packet_out);
void xqc_conn_increase_unacked_stream_ref(xqc_connection_t *conn,
                                          xqc_packet_out_t *packet_out);
...
int xqc_conn_buff_0rtt_datagram(xqc_connection_t *conn, void *data, size_t data_len,
...
xqc_gp_timer_id_t xqc_conn_register_gp_timer(xqc_connection_t *conn, char *timer_name,
...
xqc_int_t xqc_conn_gp_timer_set(xqc_connection_t *conn, xqc_gp_timer_id_t gp_timer_id,
                                xqc_usec_t expire_time);

xqc_int_t xqc_conn_gp_timer_unset(xqc_connection_t *conn, xqc_gp_timer_id_t gp_timer_id);

xqc_int_t xqc_conn_gp_timer_get_info(xqc_connection_t *conn,
                                     xqc_gp_timer_id_t gp_timer_id, xqc_bool_t *is_set,
                                     xqc_usec_t *expire_time);
...
xqc_int_t xqc_conn_send_ping_internal(xqc_connection_t *conn, void *ping_user_data,
                                      xqc_bool_t notify);
...
xqc_int_t xqc_conn_add_path_cid_sets(xqc_connection_t *conn, uint32_t start,
                                     uint32_t end);
xqc_msec_t xqc_conn_get_queue_fin_timeout(xqc_connection_t *conn);
void xqc_conn_set_init_idle_timeout(xqc_connection_t *conn,
